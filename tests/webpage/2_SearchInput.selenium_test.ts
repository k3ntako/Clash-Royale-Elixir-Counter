...
describe("CardSearch", (): void => {
  ...
  it('should have input where the "value" is updated with the user typing', async (): Promise<
    void
  > => {
    const cardSearch = await findShadowRootElement(
      homepage,
  ...
  it("should suggests cards based on search term (case insensitive)", async (): Promise<
    void
  > => {
    const cardSearch = await findShadowRootElement(
      homepage,
    ...
    const suggestions = await cardSearch.findElements(
      By.className("suggestion")
    );
    assert.lengthOf(suggestions, 4, "should have 4 suggestions");

    const cardNamePromises = suggestions.map(
      async (suggestion) => await suggestion.getText()
    );
  ...
  it("should only suggest first 20 suggestions in alphabetical order", async (): Promise<
    void
  > => {
    const cardSearch = await findShadowRootElement(
      homepage,
  ...
  it('should should push a result to top if result starts with the query (test with "z")', async (): Promise<
    void
  > => {
    const cardSearch = await findShadowRootElement(
      homepage,
  ...
  it('should should push a result to top if result starts with the query (test with "g")', async (): Promise<
    void
  > => {
    const cardSearch = await findShadowRootElement(
      homepage,
  ...
  it('should should push a result to top if result starts with the query (test with "S")', async (): Promise<
    void
  > => {
    const cardSearch = await findShadowRootElement(
      homepage,
