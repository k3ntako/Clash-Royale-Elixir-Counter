...
describe('Played cards', (): void => {
  ...
  it('should play a card if clicked on but should not add another copy to the playedCards list', async (): Promise<void> => {
    // set elixir to 10
    await buttons[10].click();
  ...
  it('should not add card to playedCards if there is not enough elixir', async (): Promise<void> => {
    // set elixir to 0
  ...
  it('should display error message if there is not enough elixir', async (): Promise<void> => {
    // set elixir to 0
    await buttons[0].click();
    ...
    const error = await findShadowRootElement(homepage, By.className('notification'));
    assert(error);
    const errorMessage = await error.getText();
    assert.strictEqual(errorMessage, "Not enough elixir");
  }).timeout(6500);

  it('should shake elixir text and turn it red if there is not enought elixir', async (): Promise<void> => {
    // set elixir to 0
    await buttons[0].click();
